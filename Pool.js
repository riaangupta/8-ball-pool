...
class Canvas {
  ...
  drawIndicator({
    ...
    let lineMinY = this.cueY + BALL_DI * 1.2 * this.angleSin; // coordinates for showing power

    let lineMaxX = lineMinX + maxDistance * this.angleCos;
    let lineMaxY = lineMinY + maxDistance * this.angleSin; // coordinates for calculating power

    let newX = lineMinX + power * maxDistance * this.angleCos;
    let newY = lineMinY + power * maxDistance * this.angleSin; // setting the force relative to power
...
class Game {
  ...
  handleTurnEnd() {
    ...
    let isCue = pocketed.filter(b => b.cue).length > 0,
        isEight = pocketed.filter(b => b.eight).length > 0; // determining valid first contact
